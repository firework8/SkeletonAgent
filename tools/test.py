...
from skeletonagent.datasets import build_dataloader, build_dataset
from skeletonagent.models import build_model
from skeletonagent.utils import cache_checkpoint, mc_off, mc_on, test_port
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='skeletonagent test (and eval) a model')
...
def inference_pytorch(args, cfg, data_loader):
    ...
    if hasattr(model.backbone, 'model_text_dict'):
        del model.backbone.model_text_dict
        model.backbone.model_text_dict = nn.ModuleDict()

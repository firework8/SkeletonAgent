...
class CustomCheckpointHook(CheckpointHook):
    ...
    def _save_checkpoint(self, runner):
        ...
        original_model_text_dict = None
        if hasattr(runner.model.module.backbone, 'model_text_dict'):
            original_model_text_dict = runner.model.module.backbone.model_text_dict
            runner.model.module.backbone.model_text_dict = nn.ModuleDict()
        
        optimizer_state = runner.optimizer.state_dict()
        original_state = optimizer_state['state']
        optimizer_state['state'] = {}
        runner.optimizer.load_state_dict(optimizer_state)
        
        super()._save_checkpoint(runner)
        
        if original_model_text_dict is not None:
            runner.model.module.backbone.model_text_dict = original_model_text_dict
        
        optimizer_state['state'] = original_state
        runner.optimizer.load_state_dict(optimizer_state)
"""
*******************
"""
...
def train_model(model,
    ...
    ******************
    Current Code:
    To save checkpoint space, the CLIP-related parts have been removed.
    You can choose to restore them to the above normal saved state.
    ******************
    """
    checkpoint_config = cfg.checkpoint_config
    if isinstance(checkpoint_config, dict):
        checkpoint_config['type'] = 'CustomCheckpointHook'
    else:
        checkpoint_config = dict(type='CustomCheckpointHook', **checkpoint_config)
    runner.register_training_hooks(cfg.lr_config, optimizer_config,
